...
| `cancel_job` | 请求取消任务 |
| `list_candidates` | 查询候选片段和审核用代理视频路径 |
| `review_candidate` | 写入确认 / 排除状态 |
| `update_clip_range` | 修改片段起止时间 |
| `export_clips` | 只导出人工确认的片段，支持单独或合并导出 |
| `start_export` | 异步导出人工确认的片段，返回可由 `get_job` 查询的任务 |
...
当前已可运行的最小闭环命令：`hello`、`create_project`、`update_project_settings`、`open_project`、`list_recent_projects`、`inspect_video`、`link_video`、`relink_video`、`extract_preview`、`create_proxy`、`save_roi`、`start_analysis`、`retry_analysis`、`cancel_job`、`get_job`、`get_active_jobs`、`list_candidates`、`review_candidate`、`update_clip_range`、`export_clips`、`start_export`、`retry_export`、`get_statistics`、`set_telemetry_consent`、`cleanup_artifacts`。

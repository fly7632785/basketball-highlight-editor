...
3. `check_runtime.py` 能确认 SQLite Schema 已随运行时打包；
4. 把生成的 `.app` 复制到没有仓库、没有 Homebrew、没有 `.venv` 的干净 macOS 用户目录；
5. 能完成 `hello → inspect_video → start_analysis → export_clips`；
6. 完成签名、公证和 Gatekeeper 安装验证。

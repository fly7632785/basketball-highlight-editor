...
标准模式包含原视频回源精筛，质量优先；快速模式使用低成本代理并跳过精筛，速度优先，可能漏检。两种模式的审核、手动调整和导出规则相同。详情见 [`DECISIONS_V1.md`](DECISIONS_V1.md)。
...
不要在公开 Issue 中发布可利用细节、视频样本、密钥或个人数据。优先使用仓库启用的 GitHub 私密漏洞报告入口；在未启用前，先联系维护者确认安全的提交方式。
